...
if __name__ == "__main__":
    ...
    clearml_logger = ClearMLLogger(task_name="Izhikevich AdamW Fast Spiking", project_name="Masterarbeit")
    clearml_logger.get_task().connect(config)
    model = IzhikevichNet(num_steps=config["num_steps"], num_input=28 * 28).to(device)
    ...
    optimizer = torch.optim.AdamW(model.parameters(), lr=0.005, betas=(0.9, 0.999))
    criterion = loss
