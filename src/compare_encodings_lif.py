...
class SimpleFCNet(nn.Module):
    def __init__(self, beta, num_steps, layer_config, reset_mechanism='zero', **kwargs):
        super().__init__()
        ...
        for i, layer in enumerate(layer_config[1:]):
            if isinstance(layer, int):
                ...
                if i == len(layer_config) - 2:
                    layers.append(
                        snn.Leaky(beta=beta, init_hidden=True, output=True, reset_mechanism="none", log_spikes=False)
                    )
                else:
                    layers.append(snn.Leaky(beta=beta, init_hidden=True, output=False, reset_mechanism=reset_mechanism,
                                            log_spikes=True))
                num_in = layer
